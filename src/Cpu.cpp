...
template <uint8_t pos> uint8_t digit(uint16_t opcode) {
...
void Cpu::tick() {
  switch (uint16_t opcode = fetch_opcode(); opcode & 0xF000) {
  case 0x0000: {
    switch (opcode) {
    case 0x0000: {
      return;
  ...
  case 0x8000: {
    ...
    case 0x8005: {
      bool safe = registers[digit<2>(opcode)] >= registers[digit<1>(opcode)];
      registers[digit<2>(opcode)] -= registers[digit<1>(opcode)];
      registers[0xF] = safe;
      break;
    }
    case 0x8006: {
      ...
      uint8_t lsb = registers[digit<2>(opcode)] & 0x01;
      registers[digit<2>(opcode)] >>= 1;
      registers[0xF] = lsb;
      break;
    }
    case 0x8007: {
      bool safe = registers[digit<1>(opcode)] >= registers[digit<2>(opcode)];
      registers[digit<2>(opcode)] =
          registers[digit<1>(opcode)] - registers[digit<2>(opcode)];
      registers[0xF] = safe;
      break;
    }
    case 0x800E: {
      ...
      uint8_t msb = (registers[digit<2>(opcode)] & 0x80) >> 7;
      registers[digit<2>(opcode)] <<= 1;
      registers[0xF] = msb;
      break;
    }
  ...
  case 0xD000: {
    uint8_t starting_x = registers[digit<2>(opcode)] % 64;
    uint8_t starting_y = registers[digit<1>(opcode)] % 32;
    uint8_t rows = digit<0>(opcode);

    bool flipped = false;

    for (int i = 0; i < rows; i++) {
      uint8_t y = starting_y + i;
      if (y >= 32) {
        break;
      }

      uint8_t row = memory[i_register + i];

      for (int j = 0; j < 8; j++) {
        uint8_t x = starting_x + j;
        if (x >= 64) {
          break;
        }

        bool on = (row >> (7 - j) & 0b1) == 0b1;
  ...
  case 0xF000: {
    ...
    case 0xF033: {
      uint8_t num = registers[digit<2>(opcode)];

      uint8_t hundredths = num / 100;
      uint8_t tenths = (num - hundredths * 100) / 10;
      uint8_t ones = num - hundredths * 100 - tenths * 10;
...
uint16_t Cpu::fetch_opcode() {
  uint8_t one = memory[program_counter++];
  uint8_t two = memory[program_counter++];
